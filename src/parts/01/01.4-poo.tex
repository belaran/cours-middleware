\mysubsection{Caractéristiques de la Programmation Orienté Objet}
...
\ifbook{
  ...
  \paragraph{} La paradigme de la programmation orientée objet se retrouve dans  plusieurs
  problématiques liées au \textbf{Middleware}, il est important d'expliciter quelques unes de ses
  caractéristiques.

  \mysubsubsection{Programmation impérative}

  \paragraph{} En premier lieu, il faut savoir que la programmation orientée objet se définit
  ...
  \paragraph{} Prenons l'exemple de traitement procédurale suivant:

  \bizProcessExemple

  \paragraph{} Une approche impérative consisterait ici à réaliser un programme réalisant chacune de
  ces actions, dans l'ordre. C'est une approche tout à fait valide et elle ne pose pas, en soi, de
  problème.

  \mysubsubsection{Approche par "objet"}
...
\ifslide {
  ...
  \begin{frame}
    \begin{center}
      \includegraphics[scale=0.30]{img/object-and-methods.png}
    \end{center}
  \end{frame}

  \mysubsection{Programmation impérative versus programmation orienté objet}
  \begin{frame}
...
\ifbook{
  ...
  processus métier - l'ajout d'un nouvel employé dans une équipe, nous allons désormais définir
  plusieurs \textbf{objets}.

  \paragraph{} Le premier objet sera l'objet salarié. Ce dernier résultera de la création du salarié
  ...
  \paragraph{} Le second objet sera en fait une collection de salariés que forme son équipe. Pour
  réaliser le second point de notre traitement, il suffira donc d'ajouter le salarié créé juste avant
...
\ifbook{
  ...
  \pageref{biz-process-code}, ce style de programmation qu'est la programmation orientée objet
  ...
  objets "métiers" décrivant de manière isolée et unitaire, les différents traitements spécifiques au
  métier de l'entreprise ou l'organisation. Les différentes applications pourront ensuite aisément
