...
plus au niveaux possibles, pour permettr de créer des composants les plus génériques possibles,
pouvant donc être aisément réutilisé, et configuré pour répondre à des problématiques similaires.
Une de ces abstractions les plus courament utilisée est la notion de \textbf{ressource}\footnote{Là
encore, l'expérience très orientée Java/JEE de l'auteur de ce document influence peut être la
sémantique. Fort heureusement, les concepts décrits ici devraient pouvoir s'appliquer à toutes
autres technologies.}.

\paragraph{} Cette notion de ressource désigne en fait une application tierse, un service au sens le
...
relationnelle. Une simple chaîne de caractère, au sein d'un programme, peut même être considéré
comme une \textbf{ressource}.

\paragraph{} Les entités désignées par ce terme sont volontairement très abstrait pour permettre de
aisément utiliser différents élements ou mécanisme de manière similaire. Mais, aussi abstrait soient
elles, les ressources partage tout de même certains point en communs:

\begin{description}
  \item[processus externe] \paragraph{} une \textbf{ressource} est forcèment quelques chose d'externe au
  processus de l'appication. Un cache local, par exemple, même s'exécutant dans un processus séparé,
  ne peut pas réellement être considéré comme \textbf{ressource}
    \paragraph{} Si le cache est dans une application séparée - par exemple une instance de
    \mylink{TODO}{MemCached} s'exécutant sur un autre serveur que l'application, il peut alors être
...
\ifbook{
  ...
  \paragraph{} La notion de \textbf{ressource} étant défini - dans ses grandes lignes, nous allons
  maintenant évoqué brièvement un mécanisme qui leur sont très souvent associé : les gestionnaires
  de connexion (souvent désigné par leur appelation anglaise de \textit{pool} de connexion).

  \paragraph{} Pour comprendre la raison de leur existence, il est nécessaire de comprendre que
  l'établissement d'une connexion à une resource - encore une fois qu'il s'agit d'une base de
  ...
  disposer d'un \textit{pool} de connexion.

  \paragraph{} Ce dernier consiste en effet en ensemble de connexion pré établis. À chaque requête,
  l'application demande au gestionnaire de connexion de lui fournir une connexion par laquelle elle
  accèdera à la ressource. Une fois son travail effectué, elle restitue au gestionnaire la
  connexion, qui pourra donc de nouveau la transmettre à un autre processus - qu'il s'agisse de la
...
\ifbook{
  \paragraph{} Comme pour beacoup des thèmes abordé dans ce cours, des ouvrages entiers pourraient
  ...
  données. L'exécution des requêtes SQL - utilisées pour manipuler ces données, sont elles
  encapsulées d'une connexion à cette base de données, ce qui aboutit naturellement à faire des
  bases de données un cas parfait de \textbf{ressource} tel que nous l'avons défini un peu plus
  haut.


  \paragraph{} L'extrait de code Java ci dessous présente la manière dont une application Java
  accède, de manière standard à une \textbf{source de  données} - nom générique donnée aux bases de
  données dans le monde Java, pour une application JEE:

  \begin{center}
    \includegraphics[scale=0.3]{img/datasource-connection.png}
  \end{center}

  \paragraph{} Il est important de remarque que là encore, le souci d'abstraction apparait
...
\ifbook {
  ...
  d'un annuaire, dont le schéma et le protocole suivant, la plupart du temps, le standard
  ...
  \paragraph{} Les notions sous jacentes au ressource ayant été amplement démontrées à ce point
  qu'il n'est pas pertinent de présenter la configuration de l'annuaire en tant que telle.
}
...
\ifbook{
  ...
  \paragraph{} Commençons d'ailleurs par souligner que ces dernières ne construisent pas en
  opposition au modèle SQL - comme leur nom pourrait sous entendre, mais comme une alternative ou
  plus souvent un complètement à l'usage de base de données SQL. En effait, l'acronyme
  \textit{NoSQL} signifie en \textit{"Not Only SQL"\footnote{Traduction: Pas seulement du SQL}}.

  \paragraph{} Ces ressources utilisent d'autres mécanismes pour effectuer la persistance et/ou la
  recherche de données. Parmi les plus courants, nous pouvons nommer les suivants:
  ...
  certainement se révéler judicieux et apporter une approche plus adapté à la problématique
